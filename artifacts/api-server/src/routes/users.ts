...
// GET /api/users/username-check?username=xxx — check availability + AI suggestions
router.get('/users/username-check', async (req, res) => {
  try {
    ...
    const usersCollection = await getUsersCollection();
    const escaped = clean.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, (c) => '\\' + c);
        ...
        const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              { role: 'user', content: '"' + clean + '" is taken. Suggest 3 creative alternatives.' },
            ],
    ...
    // Vet suggestions are available
    const vetted = await Promise.all(suggestions.map(async (s) => {
      const e2 = s.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, (c) => '\\' + c);
      const r2 = new RegExp('^@?' + e2 + '$', 'i');
      const ex = await usersCollection.findOne({ username: { $regex: r2 } });
      return ex ? null : s;
    }));
...
function fallbackSuggestions(base: string): string[] {
  ...
  return [
    base + (Math.floor(Math.random() * 998) + 1),
