  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-center px-7 py-4 min-h-0">
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.06 }} className="flex flex-col gap-3">

          {/* Google */}
          <motion.button
            whileTap={{ scale: 0.97 }} onClick={handleGoogleLogin}
            disabled={googleLoading || isLoading}
            className="w-full h-[52px] flex items-center justify-center gap-3 bg-secondary/50 rounded-2xl font-semibold text-[14px] hover:bg-secondary disabled:opacity-50 transition-colors">
          ...
          {/* Forgot password */}
          <button onClick={() => setAppPhase("auth-forgot-password")}
            className="text-right text-[12px] text-muted-foreground hover:text-foreground transition-colors font-medium">
            Forgot password?
          </button>
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
        ...
        className="shrink-0 px-7 pb-8 pt-2">
        <motion.button whileTap={{ scale: 0.97 }} onClick={handleLogin}
          disabled={!canSubmit || isLoading || googleLoading}
